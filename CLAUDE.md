...
- **HORARIOS ORGANIZADOS POR MES/TRAMO DE FECHAS:** ✅ Hecho (Agosto 2026). Cada `Horario` tiene ahora `fechaInicio?: Date` / `fechaFin?: Date` opcionales (`src/types/index.ts`) — el "tramo de fechas" en que ese horario está vigente (ej. un trimestre completo), no una semana concreta: sigue siendo la misma plantilla semanal reutilizable de siempre, solo que ahora sabe durante qué periodo aplica. Decisión tomada explícitamente con el usuario (rango flexible vs. una semana por horario) antes de implementar.
  - `HorarioManager.tsx` reescrito: el listado ya no es una lista plana, se agrupa por mes escolar (`MESES` en `constants.ts`, ahora incluye Julio: Septiembre→Julio, 11 meses) usando el mes de `fechaInicio`; dentro de cada mes los horarios salen ordenados cronológicamente, cada uno mostrando su tramo de fechas formateado (ej. "Del 8 sep al 20 dic 2026"). Los horarios sin fecha (datos antiguos de antes de este cambio, o creados sin fecha) van a una sección aparte "Sin periodo asignado" al principio — **no desaparecen**.
  - Al crear un horario nuevo, `fechaInicio` se rellena por defecto con la fecha de hoy y `fechaFin` con el 31 de julio del curso escolar activo (editable, y `fechaFin` es opcional/queda abierto si se borra).
  - De paso se eliminó la triplicación de código que arrastraba este archivo desde el principio (el formulario de crear/editar horario estaba copiado 3 veces, una de ellas en un diálogo de creación duplicado solo para el estado vacío): ahora hay un único diálogo de creación (el botón "+ Nuevo horario" del estado vacío usa el mismo) y un único componente `HorarioFormFields` reutilizado en creación y edición.
