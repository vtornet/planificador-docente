...
- **COLORES DE ASIGNATURA TAMBIÉN EN EL PDF:** ✅ Hecho (Agosto 2026). El usuario preguntó si el color se veía en el PDF exportado — no, `HorarioPDFDocument` pintaba todas las celdas igual. Añadidos valores hex (`bg`/`texto`) a cada entrada de `PALETA_ASIGNATURAS` (`src/types/constants.ts`), ya que `@react-pdf/renderer` no entiende clases de Tailwind ni tiene modo oscuro (es papel: siempre la variante clara). Cada celda del PDF ahora aplica `backgroundColor`/`color` según `celda.color`. Verificado con `pdfjs-dist` extrayendo los operadores de color reales del PDF generado: coinciden exactamente (byte a byte) con los hex esperados para Matemáticas (azul) e Inglés (cian).
