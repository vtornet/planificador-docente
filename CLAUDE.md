...
- **VISTA PREVIA ANTES DE EXPORTAR (RESUELTO):** ✅ Hecho (Agosto 2026). Uno de los 4 checkboxes sin marcar de la Fase 6 original (CSS `@media print`, vista previa, encabezados/pies de página configurables, marca de agua). Preguntado explícitamente por cuáles de los 4 seguían haciendo falta, el usuario solo pidió la vista previa — los otros 3 quedan sin implementar: no está claro que sigan aportando algo ahora que existe un sistema de exportación a PDF completo, por módulo y con cabeceras propias (construido bastante después de la Fase 6 original), y nadie ha pedido concretamente CSS de impresión de la pantalla en sí, cabeceras personalizables o una marca de agua.
  ...
  - **`src/components/export/PdfPreviewDialog.tsx`** (nuevo): recibe el PDF ya generado (en memoria) y lo muestra en un `<iframe src="blob:...">` con botones "Cancelar" y "Descargar" — la descarga real (`saveAs`) solo ocurre al confirmar. El `blob:` URL se revoca (`URL.revokeObjectURL`) al cerrar, para no acumular memoria.
  ...
  - **Tests actualizados** (`e2e/export.spec.ts`): los 2 tests que antes esperaban una descarga inmediata al pulsar la opción del menú se actualizaron para pasar primero por la vista previa (comprobando que el `<iframe>` apunta a un `blob:` real) antes de confirmar con "Descargar". Nuevo test permanente: cancelar la vista previa no dispara ninguna descarga. Suite completa: 35/35.
