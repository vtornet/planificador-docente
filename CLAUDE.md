...
- **HORARIOS: NAVEGACIÓN MES → SEMANA → HORARIO:** ✅ Hecho (Agosto 2026). `HorarioManager.tsx` reescrito por completo con navegación real de 3 niveles (aclarado explícitamente por el usuario tras una primera versión que solo agrupaba la lista sin navegación):
  ...
  - **Caso límite conocido (no bloqueante):** si "hoy" cae en agosto al pulsar el "+ Nuevo horario" general (no el de una semana concreta), el horario se crea con `fechaInicio` en agosto, que no es ninguno de los 11 meses del curso escolar (Septiembre→Julio) — quedaría sin aparecer en ningún mes de la navegación, aunque sigue guardado. No se ha arreglado porque es un caso raro (nadie suele crear horarios en agosto) y no se pidió; si molesta, la solución sería o bien excluir agosto del datepicker o añadirlo como mes 12 "fuera de curso".
