...
- **CUENTAS DE USUARIO, SINCRONIZACIÓN Y PAGO (Supabase + Stripe):** Punto 8 de la lista de prioridades. Cambio de arquitectura grande, con plan de alcance acordado antes de tocar código (ver `C:\Users\Deligober\.claude\plans\lucky-noodling-petal.md` para el razonamiento completo y las fases). Decisiones fijadas con el usuario: backend Supabase (Postgres + Auth + RLS + Edge Functions, no el Mongo+Railway que se había apuntado inicialmente), **suscripción anual** (cambiado de pago único a mitad de la Fase D — 19,99 €/año recomendado, competitivo frente a la agenda de papel de 7-20 €/año), prueba gratuita con tope independiente de 1 elemento por módulo (Horarios, Reuniones, Notas, Planificación semanal, Agenda), registro obligatorio antes de crear cualquier cuaderno, hosting en GitHub Pages sin cambios (toda la lógica de servidor vive en Edge Functions).
  ...
  - **Riesgos abiertos, anotados en el plan, sin resolver todavía:** pérdida de datos en ediciones concurrentes en dos dispositivos offline a la vez (gana el cuaderno completo más reciente, no hay merge por campo); elementos creados offline por encima del tope de prueba solo fallan en silencio (`console.error`) al sincronizar, sin aviso visible en la UI todavía; falta página de política de privacidad y borrado de cuenta antes de un lanzamiento real; la suite de Playwright (`e2e/`) está rota desde la Fase B — todos los tests pasan por `crearCuaderno()`, que ahora se topa con la pantalla de login — arreglarlo requiere decidir cómo autenticar usuarios de prueba contra Supabase (confirmación de email desactivada para cuentas de test, o creación vía API ya confirmada), deliberadamente no improvisado.
  - **Pendiente antes de un lanzamiento real — SMTP propio para los emails de Auth:** Supabase manda los emails de confirmación/recuperación de contraseña con un servicio compartido pensado solo para pruebas, con un límite de envíos muy bajo — ya se agotó una vez en esta sesión de pruebas ("email rate limit exceeded") sin que fuera un fallo del código. Antes de tener usuarias reales hay que configurar SMTP propio en Supabase → Authentication → Email (Settings → SMTP Settings) apuntando a un proveedor real. **El usuario ya tiene cuenta de pago en Resend**, así que no hace falta decidir proveedor, solo conectar las credenciales cuando toque.
...
- **ASISTENTE DE IA (Fase 9, V1) — código completo, despliegue pendiente:** Último punto de la lista de prioridades. Alcance V1 decidido con el usuario: solo para cuentas con suscripción activa (incentivo para pasar de la prueba gratuita), API **Groq** gratuita (comparada explícitamente contra Gemini: 14.400 peticiones/día compartidas frente a 100-1.000 según modelo, más rápida, y sin la cláusula de "se puede usar para entrenar" que sí tiene la capa gratuita de Gemini), interfaz de **chat flotante** (no una sección nueva de navegación), y solo 2 de los 4 módulos que planteaba el roadmap original — **Notas** y **Planificación** (Horarios y Reuniones quedan para una V2). Plan completo razonado en `C:\Users\Deligober\.claude\plans\lucky-noodling-petal.md`.
  ...
  - **Pendiente, requiere que el usuario lo haga él mismo:** crear cuenta gratuita en [console.groq.com](https://console.groq.com) y sacar `GROQ_API_KEY`; desplegar la función (`npx supabase login` una vez, luego `functions deploy`); configurar el secret `GROQ_API_KEY`; desactivar "Enforce JWT Verification" para `ai-assistant` en el panel (mismo motivo que las otras dos funciones — el preflight `OPTIONS` nunca lleva JWT); y probar en vivo con una cuenta con suscripción activa.
  ...
  - **Verificado:** `tsc`/`npm run build` limpios. Sin desplegar todavía — pendiente la prueba en vivo end-to-end (botón bloqueado sin suscripción, chat funcionando con suscripción activa, aviso de sin conexión).
