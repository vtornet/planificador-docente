...
- [ ] **11. Cancelar la suscripción sin borrar la cuenta.** Hoy el único autoservicio es el borrado completo de cuenta (irreversible, borra todos los datos); cancelar conservando los datos requiere escribir a soporte. Necesitaría un portal de facturación de Stripe (o equivalente) — ver la corrección de `TerminosUso.tsx` en "DOCUMENTOS LEGALES" más abajo, que ya documenta esta limitación con honestidad.
- [ ] **12. Confirmar el botón atrás físico de Android en un dispositivo real.** El mecanismo (History API) está verificado con Playwright (`page.goBack()`, mismo evento que dispara el gesto físico), pero nunca se ha confirmado visualmente en un móvil Android real. Ver "BOTÓN ATRÁS FÍSICO DE ANDROID" más abajo.
- [ ] **13. Volver a medir Lighthouse Performance.** La última medición conocida (~52-53) es anterior al code-splitting del bundle y a la poda del cliente de Supabase (Storage/Realtime fuera) — probablemente ya ha mejorado bastante, pero no se ha vuelto a comprobar con una auditoría real.
- [ ] **14. Ficha de alumnado (evaluación individual).** Alcance ya decidido con el usuario (16-08-2026): Nivel 1+2 de datos, sin Nivel 3 (categoría especial RGPD), con seudonimización (nombre real local-only, sin sincronizar). Sigue sin implementarse — haría falta diseñar el modelo de datos (`Alumno`) antes de tocar código. Es la funcionalidad nueva más grande de esta lista.
...
- [ ] **19. Campos opcionales del perfil, nunca pedidos:** email/contacto, etapa educativa, asignatura(s), foto o avatar.
...
**Estado real (Agosto 2026):** E2E con Playwright instalado de verdad y con tests reutilizables en el repo (ver "FRAMEWORK DE TEST E2E CON PLAYWRIGHT" y "SUITE E2E ARREGLADA TRAS LA PUERTA DE LOGIN" más arriba: `playwright.config.ts` + `e2e/*.spec.ts`, 44 tests, `npm run test:e2e`). Cada test se autentica con una cuenta Supabase efímera propia (ver `e2e/fixtures.ts`/`testUser.ts`), así que la suite corre contra el proyecto Supabase real sin dejar residuos. Vitest instalado también (ver "TESTS UNITARIOS CON VITEST" más arriba: bloque `test` en `vite.config.ts`, `src/**/*.test.ts`, 56 tests, `npm run test:unit`), cubriendo las utilidades puras de fechas/festivos/recurrencia/recordatorios/perfil/horarios y la fusión de cuadernos. Chromatic sigue sin instalar.

- **Unit Testing:** Vitest para utilidades puras — ✅ instalado y con 56 tests (`fechas.ts`, `festivos.ts`, `recurrencia.ts`, `recordatorios.ts`, `perfil.ts`, `horarios.ts`, `mergeCuaderno.ts`); sin tests de componentes React (no se ha instalado `@testing-library/react`, no se pidió)
- **E2E Testing:** Playwright para flujos críticos — ✅ instalado y con 44 tests (onboarding, navegación desktop/móvil, Horarios incl. eliminar por semana, Calendario incl. eventos recurrentes, Reuniones, Notas, exportación PDF/JSON con vista previa y compartir PDFs, offline, botón atrás de Android, reclamar cuadernos locales sin cuenta, fusión de cuadernos entre dos dispositivos, aviso de tope de prueba al sincronizar, borrado de cuenta con autoservicio, asistente de IA V2 incl. exportar a distintos destinos)
