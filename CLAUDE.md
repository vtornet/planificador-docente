...
- **AMPLIAR LA FECHA DE UN HORARIO BORRABA TODOS LOS DATOS (RESUELTO):** ✅ Hecho (Agosto 2026). El usuario reportó que, al editar un horario para ampliar su periodo de fechas (hacia delante o hacia atrás), se perdía todo el contenido, como si se hubiera creado uno nuevo. Investigando, el bug no era solo al ampliar fechas: **ocurría al guardar el diálogo "Editar horario" pase lo que pase**, aunque solo se cambiara el nombre.
  ...
  - **Encontrado de paso, sin tocar (posible bug latente, no pedido esta vez):** durante la prueba, la pregunta de "¿todo el periodo o solo esta semana?" apareció en un caso donde el rango del horario coincidía exactamente con la semana mostrada — indicio de un desajuste de zona horaria entre `new Date('2026-09-07')` (fechas de un `<input type="date">`, que el navegador interpreta en UTC) y las fechas de la semana calculadas con `date-fns` (construidas en hora local), que pueden diferir en unas horas y desplazar comparaciones de "día exacto" cerca de la medianoche. Podría afectar también a la comparación de festivos/vacaciones si el desfase cruza la medianoche. No se ha investigado a fondo ni corregido — queda anotado para revisar en otra sesión si se detectan más síntomas (ej. "se me ha marcado como festivo un día que no toca", o la pregunta de alcance apareciendo cuando no debería). **Actualización:** se ha reproducido de forma consistente en dos sesiones de pruebas distintas con el mismo rango exacto lunes-viernes, así que parece un desajuste sistemático (no un caso raro) — más motivo para revisarlo pronto.
