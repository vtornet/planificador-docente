...
- **ASIGNATURAS PREDEFINIDAS EN HORARIOS:** ✅ Hecho (Agosto 2026). `CeldaHorarioDialog.tsx` tiene dos modos:
  ...
  Se añadió `nota?: string` a `CeldaHorario` (`src/types/index.ts`). **La celda en la tabla (`HorarioTable.tsx`) solo muestra la asignatura recortada a una línea (`truncate`) + un icono pequeño si hay nota — nunca el texto de la nota.** Esto es intencional: antes la nota se previsualizaba en la celda y una nota larga ensanchaba la columna y rompía la tabla; ahora la tabla usa `table-fixed` con anchos de columna fijos y el contenido completo solo se ve en el modal (modo Ver). Solo implementado en Horarios (no en Calendario/planificación semanal, que es contenido más libre, no basado en asignaturas fijas).
