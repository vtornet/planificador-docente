...
- **RENDIMIENTO: AUDITORÍA DEL BUNDLE PRINCIPAL (Agosto 2026)**, retomando el punto de performance dejado pendiente en el code-splitting de arriba (Lighthouse Performance ~52-53). Analizado con `rollup-plugin-visualizer` en modo `raw-data` (instalado solo temporalmente con `--no-save`, sin tocar `package.json` — herramienta de diagnóstico puntual, no una dependencia permanente del proyecto) para ver exactamente qué compone el chunk principal (`index-*.js`, 714 KB / 215 KB gzip en ese momento), en vez de adivinar.
  ...
  - **Hallazgo mayor, sin resolver a propósito — pendiente de decisión del usuario:** `@supabase/supabase-js` (via `createClient()`) empaqueta SIEMPRE sus 5 sub-clientes (Auth, Postgrest, Functions, **Storage** y **Realtime**) en el chunk principal, aunque Docenza nunca usa `.storage` (no hay subida de ficheros a Supabase, las imágenes de notas van en base64 dentro del propio documento) ni `.channel()`/suscripciones en tiempo real (la sincronización es "pull" bajo demanda vía `reconcileCuadernosConSupabase`, no push). Confirmado con el análisis del bundle: `@supabase/storage-js` (106 KB) y `@supabase/realtime-js` + su dependencia `phoenix` (~155 KB) van enteros al chunk principal sin usarse nunca, y encima arrastran los polyfills de Node `Buffer`/`global` (que si no, solo harían falta en el chunk de `pdf.tsx`) porque el propio código de esos dos sub-clientes los referencia. En total, unos 260 KB sin minificar de código muerto — la mayor palanca encontrada, muy por encima del ahorro de ExportMenu.
    - **Por qué no se ha tocado:** la única forma de evitarlo es dejar de usar `createClient()` y componer a mano un cliente mínimo con los paquetes de bajo nivel (`@supabase/auth-js` + `@supabase/postgrest-js` + `@supabase/functions-js`), sin `storage-js` ni `realtime-js`. Esto no es un patrón oficialmente soportado por Supabase, y `createClient()` hace bastante fontanería interna (refresco de sesión, propagación del `Authorization` de Auth a Postgrest y a `.functions.invoke()`) que habría que replicar a mano con riesgo real de romper algo sutil en la autenticación — un cambio de arquitectura, no un ajuste de bundling, y por eso se ha dejado para que el usuario decida si merece la pena el riesgo frente al ahorro (~260 KB sin minificar, probablemente 50-80 KB gzip) antes de intentarlo. ✅ Hecho (Agosto 2026). Punto 6 de la lista de prioridades. Hasta ahora, cada sesión que quería probar la app con Playwright lo instalaba con `npx playwright ...` puntualmente y no quedaba nada reutilizable en el repo (ni config, ni tests, ni dependencia en `package.json`) — cada verificación (offline, festivos, code-splitting...) se perdía al terminar la sesión. El usuario, preguntado por el alcance, eligió explícitamente **"Solo Playwright E2E"** (no Vitest, no Chromatic todavía — quedan pendientes en "Testing Strategy" más abajo).
