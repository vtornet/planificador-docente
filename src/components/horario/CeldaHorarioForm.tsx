...
interface CeldaHorarioFormProps {
...
// Contenido de CeldaHorarioDialog.tsx, sin el <Dialog> que lo envuelve — así
// se puede reutilizar tal cual dentro de Planificación (SemanaEditor.tsx/
// VistaSemanal.tsx, ya dentro de su propio Dialog, sin anidar dos backdrops)
// y en Horarios (CeldaHorarioDialog.tsx, que sí lo envuelve en un Dialog
// propio). Un único componente para las dos garantiza que ambos sitios
// ofrezcan siempre las mismas opciones — ver "HORARIOS Y PLANIFICAR: MISMAS
...
export function CeldaHorarioForm({ celda, onGuardar, onCerrar }: CeldaHorarioFormProps) {
  ...
  const contenidoInicial = celda?.contenido || ''
  const esPredefinidaInicial = (ASIGNATURAS_PREDEFINIDAS as readonly string[]).includes(contenidoInicial)

  const [modo, setModo] = useState<'ver' | 'editar'>(contenidoInicial ? 'ver' : 'editar')
  const [asignatura, setAsignatura] = useState(
    esPredefinidaInicial ? contenidoInicial : contenidoInicial ? OTRA : ''
  )
      ...
      <>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {celda?.color && <span className={cn('w-3 h-3 rounded-full flex-shrink-0', claseSwatch(celda.color))} />}
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Nota</label>
          <Textarea
            value={nota}
